...
function updateModeChip() {
  ...
  logoutBtn.style.display = (mode === 'live' && isAuthenticated()) ? 'block' : 'none';
}
...
async function handleSignup() {
  ...
  try {
    errorEl.style.display = 'none';
    await apiSignup(email, password, name || undefined);
...
function toggleMode() {
  if (isLiveMode()) {
    ...
    // Switch to live — check if authenticated
    if (isAuthenticated()) {
      setMode('live');
      updateModeChip();
      go('dashboard');
...
async function handleLogout() {
  ...
  updateModeChip();
  go('dashboard');
  showToast('Logged out — switched to Demo mode', 'info');
}

// Listen for auth-expired events from the API client
window.addEventListener('apex:auth-expired', () => {
  setMode('demo');
  updateModeChip();
  showToast('Session expired — switched to Demo mode', 'error');
});
...
function go(viewId) {
  ...
  // Toggle App Chrome (Sidebar / Topbar) based on view
  ...
  if (viewId === 'landing') {
    ...
    // Full viewport for landing page
    mc.style.top = '0';
    mc.style.left = '0';
    mc.style.right = '0';
    mc.style.padding = '0';
    ...
    mc.style.top = 'var(--topbar-h)';
    mc.style.left = 'var(--sidebar-w)';
...
async function submitCheckin() {
  ...
  // Always update local demo data so the UI updates immediately
  updateCheckin(energy, mood, libido, srDay);

  // If live mode, also send to the API
  if (isLiveMode() && isAuthenticated()) {
    try {
      ...
      showToast('Check-in saved to server', 'success');
    } catch (err) {
      showToast('Check-in saved locally (API error)', 'error');
...
async function submitMeal() {
  ...
  // Always update local demo data so the UI is responsive
  addMeal({ type, name, kcal, protein, fat, carbs, bonuses, time: timeStr, ingredients: name.split('+').map(s => s.trim()).filter(Boolean) });
  ...
  if (isLiveMode() && isAuthenticated()) {
    try {
      ...
      showToast('Meal saved to server', 'success');
    } catch (err) {
      showToast('Meal saved locally (API error)', 'error');
...
async function sendChat() {
  ...
  if (isLiveMode() && isAuthenticated()) {
    try {
      const data = await apiSendChat(query);
      response = data.response || data.message || getChatResponse(query);
    } catch (err) {
      response = getChatResponse(query);
    }
  } else {
    // Use local keyword-match engine
...
// On first load: start in demo mode, but SHOW landing page universally first
// Auth modal will trigger smoothly on Create Account click
go('landing');
